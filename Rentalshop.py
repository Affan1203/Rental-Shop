...
def generate_pdf(df, report_type):
    ...
    pdf.ln(5); pdf.set_font("Arial", 'B', 11); pdf.cell(190, 10, txt=f"Grand Total: {CURRENCY} {total_sum:,.2f}", ln=True, align='R')
...
if not st.session_state['logged_in']:
    ...
    with tab_dash:
        ...
        with st.container(border=True):
            ...
            st.divider()
            photo_mode = st.radio("Photo Source", ["No Photo", "Take Live Photo", "Upload"], horizontal=True)
            encoded_img = None
            ...
            st.divider()
            r_name, r_phone = st.text_input("Referred By"), st.text_input("Referral Phone")
